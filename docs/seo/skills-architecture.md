...
- **Blocker нельзя закрыть тем же рабочим скилом без нового решения `orchestrator`.** После решения задача возвращается в нужный шаг конвейера (а не «сам поставил blocker — сам же снял»).
...
**Обязательные anti-patterns (наследует КАЖДЫЙ рабочий скил):**
- не менять статус в `18-implementation-matrix.md`;
- не объявлять страницу готовой;
- возвращать только результат, `warnings` и `blockers` (не «красивый отчёт» и не самовольные решения).
...
## 7. Правила владения (анти-конфликт ответственности)

Главный риск — пересечение ответственности. Для каждого спорного объекта — один владелец
смысла и чёткая цепочка. Эти правила обязательны в `anti-patterns` соответствующих скилов.

### 7.1. Цена (критично)
Обязательно в `anti-patterns` для `copywriter`, `seo-json-ld-agent`, `product-catalog-api`:
```
...
### 7.2. Title / Description
```
semantic-core-lite   → предлагает черновик по кластеру (H1/title/description);
copywriter           → финализирует формулировку (смысл);
technical-seo-agent  → только внедряет/проверяет технически, смысл НЕ переписывает;
page-quality-gate    → проверяет длину, уникальность и соответствие странице.
```
Согласовано с `09-content-guidelines.md` §7 и ownership boundary `technical-seo-agent`.

### 7.3. URL
```
catalog-architect     → предлагает/валидирует URL по 01-sitemap-pages.md;
orchestrator          → утверждает через work-order из матрицы 18;
technical-seo-agent   → НЕ придумывает URL, ставит только canonical/sitemap;
internal-linking-agent → использует ТОЛЬКО существующие URL из матрицы 18.
```
Защита от ссылок на разные варианты одного URL (дубли/404/каннибализация).
